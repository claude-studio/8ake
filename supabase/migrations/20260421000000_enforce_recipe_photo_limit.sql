...
-- CLA-91: 동시 INSERT 직렬화 (트랜잭션 락 추가)

-- 트리거 함수: INSERT/UPDATE 전에 레시피당 사진 수가 2장을 초과하는지 검사
-- 동시 INSERT 경쟁 조건 방지를 위해 부모 recipes 행에 FOR UPDATE 락 사용
...
BEGIN
  ...
  -- 동시 INSERT 직렬화: recipe_id 단위로 트랜잭션 락을 잡아
  -- COUNT 검사와 INSERT 사이에 다른 트랜잭션이 끼어들지 못하게 한다
...
COMMENT ON FUNCTION check_recipe_photo_limit() IS '레시피당 사진을 최대 2장으로 제한한다. pg_advisory_xact_lock으로 recipe_id 단위 직렬화 후 COUNT 검사. 동시 INSERT 경쟁 조건 방지. 3번째 INSERT 또는 다른 레시피로의 UPDATE 시 check_violation 예외를 발생시킨다.';
